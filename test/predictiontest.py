...
def process_file(pgn_file_name, engine, player_name=None):
    ...
    with open(pgn_file_name) as input_file:
        while True:
            ...
            percentage = float(correct_pred_count) / float(ply_count) * 100.0 if ply_count > 0 else 0
            print(game.headers["White"] + " vs " + game.headers["Black"] + ": %.2f%%, "  % (percentage) + str(correct_pred_count) + " / " + str(ply_count))
